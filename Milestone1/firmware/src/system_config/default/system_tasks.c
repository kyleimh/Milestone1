...
void SYS_Tasks ( void )
{
    ...
    xTimer1 = xTimerCreate("Timer1", 50/portTICK_PERIOD_MS, pdTRUE, (void *)0, vTimerCallback);
    if( xTimer1 == NULL )
    {
        /* TODO: Debug... The timer was not created. */
        outputEvent(TimerNotCreated);
    }
    else
    {
        if( xTimerStart( xTimer1, 0 ) != pdPASS )
        {
            ...
            outputEvent(TimerNotActive);
        }
    }
    
    /* Create queue for storage of characters to be sent over USART */
    xQueue1 = xQueueCreate( 10, sizeof(char) );
    if( xQueue1 == 0 )
    {
        // TODO: Debug... Queue was not created and must not be used.
        outputEvent(QueueNotCreated);
    }
